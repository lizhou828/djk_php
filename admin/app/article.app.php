...
class ArticleApp extends BackendApp
{
    ...
    function index()
    {
        ...
        'conditions'  => 'article.dropState=1 and store_id=0' . $conditions,
        'limit'   => $page['limit'],
    ...
    function add()
    {
        if (!IS_POST)
        {
            ...
            $this->assign('build_editor', $this->_build_editor(array(
                'name' => 'content',
                'content_css' => SITE_URL . "/themes/mall/{$template_name}/styles/{$style_name}/css/djk.css"
            )));
   ...
   function ajax_col()
   {
    ...
    function drop()
    {
        ...
        if (!$this->_article_mod->edit("article_id in ($id)",array("dropState"=>0)))
        {
            $this->show_warning($this->_article_mod->get_error());
