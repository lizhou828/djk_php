...
class GcategoryApp extends BackendApp
{
    ...
    {
        /*取得所有频道*/
        $this->assign("channels", Conf::get('pindao_admin'));
    ...
    function add()
    {
        ...
        else
        {
            $data = array(
                ...
                'changshang_ticheng' => $_POST['changshang_ticheng']?        ($_POST['changshang_ticheng'])/100:0,
                'dailishang_ticheng' => $_POST['dailishang_ticheng']?        ($_POST['dailishang_ticheng'])/100:0,
                'lingshoushang_ticheng' => $_POST['lingshoushang_ticheng']? ($_POST['lingshoushang_ticheng'])/100:0,
                'shitidian_ticheng' => $_POST['shitidian_ticheng']? ($_POST['shitidian_ticheng'])/100:0
            );
            ...
            $_data= $this->_gcategory_mod->get($cate_id);
            if ($_data['parent_id']==0) {
                $pd_gcategory = & m("pdcategory");
                $pd_ids = $_POST['pd_id'];
                if (!empty($pd_ids)) {
                    foreach ($pd_ids as $pd_id) {
                        $pd_data = array(
                            'pd_id'=> $pd_id,
                            'category_id'=>$cate_id,
                            'sort_id'=>$_POST['sort_order'],
                        );
                        $pd_gcategory->add($pd_data);
                    }
                }
            }
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
            ...
            $pd_gcategory = & m("pdcategory");
            if ($pd_gcategory->_getpdIds($id)) {
                $this->assign('pids',$pd_gcategory->_getpdIds($id));
            }
            if (!$gcategory)
            {
        ...
        else
        {
            ...
            $cate_id= $_pdgcategory_mod->deleteBycateId($id);
    ...
    function drop()
    {
        ...
        if (!$this->_gcategory_mod->edit("cate_id in ($id)",array("dropState"=>0)))
        {
            $this->show_warning($this->_gcategory_mod->get_error());
            return;
        }
    ...
    function get_ajax_cl2() {
        ...
        if ($cate_ids) {
            foreach ($cate_ids as $cate_id) {
                if ($this->_gcategory_mod->get($cate_id)) {
                    $gcategories[]=$this->_gcategory_mod->get($cate_id);
                }
            }
        }
