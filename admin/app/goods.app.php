...
class GoodsApp extends BackendApp
{
    ...
    function index()
    {
        ...
        $goods_list = $this->_goods_mod->get_list(array(
            'conditions' => "1 = 1 and g.dropState=1" . $conditions,
            'count' => true,
            'order' => "$sort $order",
            'limit' => $page['limit'],
        ));
    ...
    function goodsView() {
        ...
        $this->import_resource(array('script' => 'mlselection.js,inline_edit.js'));
        $this->assign('enable_radar', Conf::get('enable_radar'));
        $this->display('goodsView.index.html');
    }
    ...
    function recommend()
    {
        ...
        else
        {
            ...
            if($ids) {
                $recommeded_goods_mode = & m('recommendgoods');
                foreach($ids as $key=>$goodsId) {
                    $recom_goods[$key] = $recommeded_goods_mode->get(array(
                        'conditions' => 'recom_id = '.$recom_id." and goods_id =".$goodsId,
                        'files' => 'id'
                    ));
                    if($recom_goods[$key]) {
                        $recommeded_goods_mode->drop($recom_goods[$key]["id"]);
                    }
                }
            }
   ...
   function ajax_col()
   {
    ...
    function get_ajax_price() {
        $id     = empty($_GET['id']) ? 0 : intval($_GET['id']);
        $goods = $this->_goods_mod->get_info($id);
        echo ecm_json_encode($goods);
//        echo "进货价:".$goods['org_price']."<br>零售价:".$goods['price'];
    }
}
