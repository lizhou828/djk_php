...
class ScategoryApp extends BackendApp
{
    ...
    {
        /* 取得店铺分类 */
        $scategories = $this->_scategory_mod->get_list2();
   ...
   function ajax_col()
   {
       ...
       if (in_array($column ,array('cate_name', 'sort_order', 'dropState')))
       {
           $data[$column] = $value;
           if($column == 'cate_name')
           {
    ...
    function drop()
    {
        ...
        //删除流程改造，物理删除改逻辑删除
        if (!$this->_scategory_mod->edit("cate_id in ($id)",array("dropState"=>0)))
        {
            $this->show_warning($this->_scategory_mod->get_error());
            return;
        }
...
/* 导出数据 */
    function export()
    {
        ...
        $gcategories = $this->_scategory_mod->get_list2();
