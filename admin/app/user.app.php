        ...
        if($type=="02"){
            ...
            $users = $this->_user_mod->find(array(
                ...
                'fields' => 'this.*,store.store_id,userpriv.store_id as priv_store_id,userpriv.privs,
                          (select m.user_name from ecm_member m where m.user_id = member.t_id ) as tuserName',
                'conditions' => "1=1 and member.dropState=1 and member_type='$type'" . $conditions,
                'limit' => $page['limit'],
                'order' => "$sort $order",
                'count' => true,
    ...
    function add()
    {
        ...
        else
        {
            ...
            $this->show_message('add_ok',
                'back_list',    "index.php?app=".$_GET["app"]."&type=$type",
                'continue_add', "index.php?app=".$_GET["app"]."&type=$type&amp;act=add"
            );
        }
    }
...
//				"region_id"=>$region_id,
            ...
            $this->show_message('edit_ok',
                'back_list',    "index.php?app=".$_GET["app"]."&type=$type",
                'edit_again',   "index.php?app=".$_GET["app"]."&type=$type&amp;act=edit&amp;id=" . $id
            );
        }
    }
