...
class ComuploadApp extends StoreadminbaseApp
{
    ...
    function ComuploadApp()
    {
        ...
        {
            $this->pd_id = intval($_REQUEST['pd_id']);
        }
    ...
    function uploadedfile()
    {
            ...
            if ($files['error'] === UPLOAD_ERR_OK)
            {
                ...
                $dirname=Conf::get('up_dir');

                $filename  = $uploader->random_filename();
                $file_path = $uploader->save($dirname, $user["user_id"]."_".$filename);
                /* 处理文件入库 */
...
//                }
                    ...
                    for($i = 0; $i < count($sizes); $i++){
                        ...
                        make_thumb(ROOT_PATH . '/' . $url, ROOT_PATH .'/' . $path, $size[0], $size[1], THUMB_QUALITY);
                    }
            ...
            if(preg_match("/^(http:\/\/){1,1}.+(gif|png|jpeg|jpg){1,1}$/i", $remote_url))
            {
                ...
                if ($this->instance == 'goods_image') // 如果是上传商品相册图片
                {
                    ...
                    $sizes = array(INDEX_TOP_WIDTH . 'X' . INDEX_TOP_HEIGHT, INDEX_CENTER_WIDTH . 'X' . INDEX_CENTER_HEIGHT,
                        ...
                        GOOD_MIDDLE_WIDTH.'X'.GOOD_MIDDLE_HEIGHT,
                        NEW_GOOD_BIG_WIDTH .'X' . NEW_GOOD_BIG_HEIGHT,
                        NEW_GOOD_MIDDLE_WIDTH . 'X' . NEW_GOOD_MIDDLE_HEIGHT,
                        NEW_GOOD_SMALL_WIDTH. 'X' .NEW_GOOD_SMALL_HEIGHT,
                        NEWS_GOOD_MIDDLE_WIDTH. 'X' .NEWS_GOOD_MIDDLE_HEIGHT
                    ...
                    $url =  dirname($new_url) . '/' . basename($new_url);
                    $urls = explode('.',$url);
                    for($i = 0; $i < count($sizes); $i++){
                        $path = $urls[0] . '_' . $sizes[$i] . '.' . $urls[1];
                        $size = explode('X', $sizes[$i]);
                        make_thumb(ROOT_PATH . '/' . $new_url, ROOT_PATH .'/' . $path, $size[0], $size[1], THUMB_QUALITY);
                    }
