...
class Seller_groupbuyApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        if($_GET["app"]=="service"){
              ...
              if($_GET["store_id"]!="-1" && $_GET["store_id"]!="")
              {
                  ...
                  $store_id.="(SELECT store_id FROM ".DB_PREFIX."store WHERE (fwzx=$fwzxId $query_conditions) AND tuoguan=1 AND dropState=1 $conditions2)";
        ...
        $groupbuy_list = $this->_groupbuy_mod->find(
            array(
                'join' => 'belong_goods',
                'conditions' => "gb.store_id in (".$store_id.") $filter " . $conditions,
        ...
        //$this->print_r($groupbuy_list);

        if($_GET["app"]=="service")
        {
            $this->display('service_groupbuy.index.html');
        }else{
    ...
    function add()
    {
        if (!IS_POST)
        {
            ...
            {
                if($_GET["app"] == "service" ){
                    $this->show_warning('您目前还未发布在售商品，请先<a href="index.php?app=service&act=add">发布商品</a>', 'add_goods', 'index.php?app=my_goods&act=add');
                    return;
                }
            ...
            if($_GET["app"]=="service"){
                $this->display('service_groupbuy.form.html');
            }else{
        ...
        else
        {
            ...
            if($_GET["app"]=="service"){
                $this->show_message('add_groupbuy_ok',
                    'back_list', 'index.php?app=service&amp;act=tuangou&amp;p=service',
                    'continue_add', 'index.php?app=service&amp;act=tuangou_add&amp;p=service'
                );
            }else{
                $this->show_message('add_groupbuy_ok',
