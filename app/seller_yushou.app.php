 ...
 *    @usage    none
 */
class Seller_yushouApp extends StoreadminbaseApp
{
    var $_store_id;
    var $_goods_mod;
    var $_store_mod;
    ...
    {
         $this->Seller_yushouApp();
    }

    function Seller_yushouApp()
    {
        parent::__construct();
...
//        }
        $store_id = $this->_store_id;
        $yushou_list = $this->_yushou_mod->find(
            array(
                'conditions' => "ys.store_id in (".$store_id.")",
                'order' => 'ys.id DESC',
...
//        }
        foreach ($yushou_list as $key => $yushou)
        {
            $goods = $this->_goods_mod->get($yushou['goods_id']);
            $yushou_list[$key]['goods_name'] = $goods['goods_name'];
            if(!empty($goods['default_image'])) {
                $yushou_list[$key]['default_image'] =$goods['default_image'];
            } else {
                $yushou_list[$key]['default_image'] = Conf::get('default_goods_image');
            }
        }
        /* 当前位置 */
        $this->_curlocal(LANG::get('member_center'),    'index.php?app=member',
                         LANG::get('yushou_manage'), 'index.php?app=seller_yushou',
                         LANG::get('yushou_list'));
...
//        }else{
            $this->_curitem('yushou_manage');
            /* 当前所处子菜单 */
            $this->_curmenu('yushou_list');
//        }
...
//        $this->_import_resource();
        ...
        $this->assign('yushou_list', $yushou_list);
...
//        );
        ...
        if($_GET["app"]=="service")
        {
            $this->display('service_yushou.index.html');
        }else{
            $this->display('seller_yushou.index.html');
        }
        ...
        if (!IS_POST)
        {
            ...
            if ($goods_count == 0)
            {
                $this->show_message('has_no_goods',
                    'add_goods', 'index.php?app=seller_yushou',
                    'continue_add', 'index.php?app=seller_yushou&amp;act=add'
                );
                return;
//
            }
            ...
            $this->_config_seo('title', Lang::get('member_center') . ' - ' . Lang::get('add_yushou'));
            $this->_import_resource();


            $this->display('seller_yushou.form.html');
        }
        else
        {
            /* 检查数据 */
...
//            }
                ...
                $this->show_message('新增预售信息成功',
                    'back_list', 'index.php?app=seller_yushou',
                    'continue_add', 'index.php?app=seller_yushou&amp;act=add'
                );
    ...
    function edit()
    {
        ...
        if (!$id)
        {
            $this->show_warning('no_such_yushou');
            return false;
        }
        ...
        if (!IS_POST)
        {
            ...
            $this->_curlocal(LANG::get('member_center'),    'index.php?app=member',
                             LANG::get('yushou_manage'), 'index.php?app=seller_yushou',
                             LANG::get('edit_yushou'));

            $this->_curitem('yushou_manage');


            /* 当前所处子菜单 */
            $this->_curmenu('edit_yushou');
            ...
            $this->_config_seo('title', Lang::get('member_center') . ' - ' . Lang::get('edit_yushou'));
//            $this->_import_resource();
        ...
        else
        {
            ...
            if($_GET["app"]=="service"){
                $this->show_message('edit_yushou_ok',
                ...
                $this->show_message('edit_yushou_ok',
                    'back_list', 'index.php?app=seller_yushou',
                    'continue_edit', 'index.php?app=seller_yushou&act=edit&id=' . $id
                );
            }
...
//        }
        ...
        $this->show_message('drop_yushou_successed');
    }
...
//            'max_per_user' => $post['max_per_user'],
    ...
    function _get_member_submenu()
    {
        $menus = array(
            array(
                'name'  => 'yushou_list',
                'url'   => 'index.php?app=seller_yushou',
            ),
        );
