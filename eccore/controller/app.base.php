...
class BaseApp extends Object
{
    ...
    function __construct()
    {
        ...
        /*@session_start();
        $invi_code =trim($_POST['t_id'])?trim($_POST['t_id']):trim($_GET['t_id']);
        if ($invi_code != "") {
            $_SESSION["t_id"] = $invi_code;
            ecm_setcookie("t_id",$invi_code,time() + 9999999);
        }*/
    }
    ...
    //删除指定文件
    function deleteFileInfo($fileUrl)
    {
        //先删掉文件
        @unlink($fileUrl);
        ...
        $fileDir=substr($fileUrl,0,$idx);

        //如果是空文件夹，删除空文件夹
        if($this->is_empty_dir($fileDir))      //如果是空的
        {
            @rmdir($fileDir);                   //直接删除
        }
