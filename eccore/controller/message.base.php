...
function exception_handler($errno, $errstr, $errfile, $errline)
{
    ...
    if ($errno != E_NOTICE)
    {
        ...
        if ($errno == E_USER_ERROR || $errno == E_ERROR || $errno == E_PARSE ) {
            db()->rollback_transaction(); //出现异常，数据库回滚
        }
        ...
        if ($errno == E_USER_ERROR || $errno == E_ERROR || $errno == E_PARSE || $errno == E_WARNING)
        {
            ...
            /*$error_emall=Conf::get('error_emall');            //异步发送错误信息到开发
            if(count("$error_emall")>0){
                $smtp=$GLOBALS["smtp"];
                $title = "系统出错";
                $content = "系统出错,错误信息:".$errno."<br>". $errstr."<br>".  $errfile."<br>".  $errline;
                $content .= "<br>本邮件为系统自动发送，请勿回复！";
                $smtp->send($error_emall,$title,$content);
            }*/
...
function put_log($err, $msg, $file, $line)
{
    ...
    $filename = ROOT_PATH . "/djk_log/" .date("Ymd"). ".log";

    if (!is_dir('djk_log'))
    {
        ecm_mkdir(ROOT_PATH . '/' . 'djk_log');
    }
    $handler = null;
    date_default_timezone_set('Asia/Shanghai');
    if (($handler = fopen($filename, 'ab+')) !== false)
    {
        fwrite($handler, "\r\n错误url=http://".$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"]."\r\n".date('Y-m-d H:i:s',time()) ."  " . "\t[$err]$msg\t$file\t$line\n");
        fclose($handler);
    }
}
