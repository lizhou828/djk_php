...
class ECMall
{
    ...
    function startup($config = array())
    {
        ...
        if(!IS_POST&&$_GET["t_id"]!=""){
            setcookie("t_id", '', time(), "/", "www.dajike.com");
            //@session_start();
            $_SESSION["t_id"] = $_GET["t_id"];
            ecm_setcookie("t_id", $_GET["t_id"], time() + 60*60*24);
        }
        ...
        c($app);
        $current_db = &db();
        $GLOBALS["current_db"] = $current_db;             //将数据库对象放到$GLOBALS中
        $current_db->auto_commit_flase();
        $app->do_action($act);                        //转发至对应的Action
        $app->destruct();
        $current_db->commit_transaction();
        $current_db->end_transaction();
    }
...
function &m($model_name, $params = array(), $is_new = false)
{
    ...
    if ($is_new || !isset($models[$model_hash]))
    {
        ...
        $current_db = $GLOBALS["current_db"];
        if (!$current_db) $current_db = db();
        if ($is_new)
        {
            return new $model_name($params, $current_db);
        }
        $models[$model_hash] = new $model_name($params, $current_db);
    }
...
function changerStr($value,$num){
    return $value*$num;
}

/**
 * exec php code in template by this tag
 * example：
 * <pre>
 *   in template member.showLog.html:
 *   {$log.jine|myeval:"ceil(value*40)"}  //convert the float to integer, the "value" is $log.jine
 *
 * <pre>
 * @param $value
 * @param $content
 * @return mixed
 */
function myeval($value, $content) {
    if (!is_numeric($value)) {
        $value = "'" . $value . "'";
    }
    $evalStr = str_replace("value", $value, $content) . ";";
    return eval("echo " . $evalStr);
}
...
function bank_hidden($card){
    $card = trim($card);
    return substr($card,0,6)."****".substr($card,strlen($card)-4,4);
}

function is_email($email) {
    return preg_match("/^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/", $email);
}

function is_mobile($mobile) {
    return preg_match("/^1[3|5|8][0-9]\d{8}$/", $mobile);
}
...
function number_format2($price, $price_format = NULL)
{
    if (empty($price)) $price = '0.00';
    $price = number_format($price, 2);
    return $price;
...
function time_format2($format,$time){
...
function number_format0($price, $price_format = NULL)
{
