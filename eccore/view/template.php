...
class ecsTemplate
{
     ...
     * @return  sring
     */
    function fetch2($filename, $cache_id = '')
    {
        if (!$this->_seterror)
        {
            error_reporting(E_ALL ^ E_NOTICE);
        ...
        else
        {
            if ($this->_checkfile)
            {
                if (!is_file($filename))
                {
                    $filename = ROOT_PATH. '/themes/' . $filename;
                }
            }
            else
            {
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'include_template':
                    ...
                    return '<?php echo $this->fetch2(' . "'$t[file]'" . '); ?>';
                    break;
                ...
                case 'html_options2':
                    ...
                    return '<?php echo $this->html_options2(' . $this->make_array($t) . '); ?>';
                    break;
                case 'widgets':
                    $t = $this->get_para(substr($tag, 8), 0);
                ...
                case 'html_image':
                    ...
                    return '<?php echo $this->resize(' . $this->make_array($t) . '); ?>';
                    break;
                case 'url':
                ...
                case 'r_url':
                if (IS_MYREWRITE == 'ON') {
                     $str = str_replace('', '', substr($tag, 6));
                     $str = str_replace('app=', '/', $str);
                     if(strpos($str,'&act=')) {
                         $str = str_replace('&act=', '/', $str);
                     }
                     $str = str_replace('&', '?', $str);
                     $tmp = explode('?', $str);
                     $tmp = str_replace('=','-',$tmp);
                     foreach ($tmp AS $key=>$val)
                     {
                         ...
                         if($key > 0){
                             if ((substr($a[1], 0, 1) == '$'))
                             {
                                 $tmp[$key] = "' . ". $this->get_val(substr($a[1], 1)) . ". '";
                             }else if($a[1] == ''){
                                 $tmp[$key] = 0;
                             }else{
                                 $tmp[$key] = $a[1];
                     ...
                     foreach($tmp as $key=>$val){
                         ...
                         else if($key == ($count-1)){$arr .= $val . '.html';}
                         else{$arr .= $val . '-';}
                     }
                    return '<?php echo r_url(\'' . $arr . '\'); ?>';
                  } else {
                    $str = str_replace('index.php?', '', substr($tag,6));
                    $str = str_replace('&amp;', '&', $str);
                    $tmp = explode('&', $str);
                    $arr = array();
                    ...
                    break;
                case 'img_url':
                        $image_urls = explode('/',substr($tag,8));
                        $imgDomain = $this->get_val(substr($image_urls[0],1));
                        $imgUrl = $this->get_val(substr($image_urls[1],1));
                        $size = $image_urls[2];
                        if ($imgUrl){
                        //$path = $imgDomain . '.\'/\'. explode(\'.\',' . $imgUrl . ')[0] .\'_\' . \'' . $size . '\' . \'.\' . explode(\'.\',' . $imgUrl . ')[1]';
                        $path = '$imgarr = explode(\'.\',' . $imgUrl . ');
                            if(strpos($imgarr[0],\'system\') == false)
                            echo ' . $imgDomain . ' .\'/\'.$imgarr[0] .\'_\' . \'' . $size . '\' . \'.\' .$imgarr[1];
                            else echo ' . $imgDomain . '.\'/\'.' . '$imgarr[0]. \'.\' .$imgarr[1]';
                        } else {
                            $path = $imgDomain.'/data/system/default_goods_image.gif';
                        }
                        return '<?php '. $path .'; ?>';
                    break;
                case 'ecjoin':
    ...
    function html_options2($arr)
    {
        ...
        if ($options)
        {
            foreach ($options AS $key => $val)
            {
                ...
                $parent_id = $scategories[$key]["parent_id"];

                $changshang_ticheng = $scategories[$key]["changshang_ticheng"];
                $dailishang_ticheng = $scategories[$key]["dailishang_ticheng"];
                $lingshoushang_ticheng = $scategories[$key]["lingshoushang_ticheng"];
                $shitidian_ticheng = $scategories[$key]["shitidian_ticheng"];

                $out .= $key == $selected ? "<option value=\"$key\"
                parent_id=\"$parent_id\"
                changshang_ticheng=\"$changshang_ticheng\"
                dailishang_ticheng=\"$dailishang_ticheng\"
                lingshoushang_ticheng=\"$lingshoushang_ticheng\"
                shitidian_ticheng=\"$shitidian_ticheng\"
                selected>$val</option>" : "<option value=\"$key\"
                parent_id=\"$parent_id\"
                changshang_ticheng=\"$changshang_ticheng\"
                dailishang_ticheng=\"$dailishang_ticheng\"
                lingshoushang_ticheng=\"$lingshoushang_ticheng\"
