...
class SMSClient extends BaseApp{

	var $userId 	= MSM_USERID;
	var $password	= MSM_PASSWORD;
	var $account	= MSM_ACCOUNT;
	...
	public function sendSMS($phones, $content, $postFixNumber=1, $sendTime='', $sendType=1) {
        if(CHANAL1 == "yes") {   // 是否调用短信通道1
            $rs = $this->msmChannal1Send($phones, $content);
            if ($rs == null || $rs == "") {
                $this->saveMsm($content,$phones,1,3);
                return array('errorno'=>-1, 'info'=>'发送短信请求失败');
            }
            //$this->pr($rs);
            if (explode("0:", $rs) >1) {
                $this->saveMsm($content,$phones,1,2);
                return array('errorno'=>0, 'info'=>"发送成功");
            } else {
                $this->saveMsm($content,$phones,1,3);
                return array('errorno'=>-1, 'info'=>'发送短信请求失败');
            }
        } else {                 // 默认短信通道
           $this->saveMsm($content,$phones,0,2);
           return  $this->msmDefaultSend($phones, $content, $postFixNumber, $sendTime='', $sendType);
        }
	}

    public function saveMsm($content,$phones,$sendType,$status){
        /** 保存短信内容到数据库*/
        $data = array(
            'content' => $content,
        ...
        $sendmsm_mode = &m("msm");
//        $sendmsm_mode->db->query("insert into ecm_msm (content,create_time,cellphone,sendtype,status)values('".$content."','".date('Y-m-d H:i:s',time())."','".$phones."','".$sendType."','".$status."')");
        $sendmsm_mode->add($data);
    }
    /* 短信通道1 */
    public function msmChannal1Send($phones, $content) {
        include_once(ROOT_PATH . '/includes/HttpClient.class.php');

        $params["zh"] = MSM1_USERID;
        $params["mm"] = MSM1_PASSWORD;
        $params["sms_type"] = MSM1_ACCOUNT;
        $params["nr"] = $content;
        $params["hm"] = $phones;

        $Client = new HttpClient(MSM1_URL);
        $url = "http://".MSM1_URL.":".MSM1_PORT."/jk.aspx";   // http://118.145.30.165:60001/jk.aspx   完整url
    ...
    /* 短信发送默认通道 */
    public function msmDefaultSend($phones, $content, $postFixNumber=1, $sendTime='', $sendType=1) {
        /*if ($this->TokenId) {
