...
define('SIZE_GOODS_IMAGE', '8097152');   // 商品大小限制2M
define('SIZE_STORE_LOGO', '20480');      // 店铺LOGO大小限制2OK
define('SIZE_STORE_BANNER', '2048576');  // 店铺BANNER大小限制1M
define('SIZE_STORE_CERT', '  8009600');     // 店铺证件执照大小限制2M
define('SIZE_STORE_PARTNER', '102400');  // 店铺合作伙伴图片大小限制100K
define('SIZE_CSV_TAOBAO', '10097152');     // 淘宝助理CSV大小限制2M

/* 店铺状态 */
define('STORE_APPLYING', 0); // 申请中
define('STORE_OPEN',     1); // 开启
define('STORE_CLOSED',   2); // 关闭
define('STORE_UNPASS',   3); // 审核不通过
...
/*余额log类型*/
//-1未知,0抽奖,1未抽奖，获得平均分配,2买家退款，3提现，4完成订单打入商家账户,5支付,6收益或奖励,7充值
define('UNKNOW',-1);
define('LOTTERY',0);
define('AVERAGE',1);
define('REIMBURSE',2);
...
/*积分log类型*/
//-1未知,0抽奖,1未抽奖自动分配,2帮人注册送积分,3取消订单，5支付，6推广获得积分
define('JIFEN_UNKNOW',-1);
define('JIFEN_LOTTERY',0);
define('JIFEN_AVERAGE',1);
define('JIFEN_HELP_REGISTER',2);
define('JIFEN_CANCEL_ORDER',3);
define('JIFEN_PAY',5);
define('JIFEN_TUIGUANG',6);
...
define('ACC_HOME', 'home');                     //acategory表中code字段为home时——店主之家类别
define('ACC_PAY', 'pay');                     //acategory表中code字段为pay时——支付方式类别
define('ACC_SERVICE', 'service');              //acategory表中code字段为service时——售后服务类别
define('ACC_CENTER', 'center');                     //acategory表中code字段为center时——客服中心类别
...
/*支付宝支付邮箱*/
define('SELLER_EMAIL',"pengchangguo@dajike.com");
...
EOT;
    ...
    function display($f)
    {
        ...
        $this->assign('SHOP_DOMAIN', SHOP_DOMAIN);
        $this->assign('site_url', SITE_URL);
        $this->assign('mobile_pay_url', PAY_ORDER_BY_MOBILE);
        $this->assign('img_url', IMG_URL);
        $this->assign('chat_domain', CHAT_DOMAIN);
        $this->assign('chat_url', CHAT_URL);
        $this->assign('chat_ctx', CHAT_CTX);
        $this->assign('domain', DOMAIN);
        $this->assign('is_chat', IS_CHAT);
        $this->assign('is_myrewrite', IS_MYREWRITE);
...
EOT;
    ...
    function _build_upload($params = array())
    {
        ...
        $file_size_limit = isset($params['file_size_limit']) ? $params['file_size_limit']: '8 MB'; //默认最大2M
...
class BaseVisitor extends Object
{
    ...
    function BaseVisitor()
    {
        ...
        $login_name2 = ecm_getcookie("login_name2");

        if(APP_ROOT =="APP_ROOT" && !empty($login_name2)){
            $login_name2=base64_decode($login_name2);
            include_once(ROOT_PATH . '/app/member.app.php');
            $member_mod =& m('member');
            $user = $member_mod->get("user_name = '$login_name2'");
            //$this->pr($user);
            if($user){
                $this->info         = $user;
                $this->has_login    = true;
    ...
    {
        ecm_setcookie("login_name2",base64_encode($user_info["user_name"]),time()+60*60*24 ,"/","dajike.com");
    ...
    function _get_detail()
    {
        ...
        if ($detail['store_id'] && $detail['state'] != STORE_APPLYING  && $detail['state'] !=STORE_UNPASS) // 排除申请中的店铺
        {
            $detail['manage_store'] = $detail['has_store'] = $detail['store_id'];
        }
        if($detail['state'] == 3){
            $detail['store_id'] = null;
            //$detail['store_id'] = null;
        }
        return $detail;
    }
    ...
    function logout()
    {
        ...
        ecm_setcookie("login_name2","1",time()-9999,"/","dajike.com");
    }
