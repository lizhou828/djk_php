...
$(function()
{
    ...
    $('img[ectype="flex"]').click(function(){
        ...
        if(status == 'open')
        {
            ...
            $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        "<td class='node' width='30%'><img class='preimg' src='templates/style/images/treetable/vertline.gif'/>"+src+"<span class='node_name editable' ectype='inline_edit' fieldname='cate_name' fieldid='"+res[i].cate_id+"' required='1'>"+res[i].cate_name+"</span></td>"+
            ...
            "<td class='handler' style='width:180px;'><span style='width:180px;'><a href='index.php?app=gcategory&amp;act=edit&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location=\"index.php?app=gcategory&act=drop&id="+res[i].cate_id+"\";'>"+lang.drop+"</a> | <a href='index.php?app=gcategory&amp;act=add&amp;pid="+res[i].cate_id+"'>"+lang.add_child+"</a>|<a href='javascript:attr("+res[i].cate_id+")'>"+lang.gcategory_attr+"</a></span></td></tr>";
                    }
                    //将组成的字符串添加到点击对象后面
                    pr.after(str);
                    change_background();
...
function secajax(ob)
{
    ...
    var src = $(ob).attr('src');
        if(status == 'open')
        {
            ...
            "<td class='align_center'><span class='editable' ectype='inline_edit' fieldname='sort_order' fieldid='"+res[i].cate_id+"' datatype='number'>"+res[i].sort_order+"</span></td>"+
            "<td class='align_center'>"+if_show+"</td>"+
            "<td class='handler' style='width:180px;'><span style='width:180px;'><a href='index.php?app=gcategory&amp;act=edit&amp;id="+res[i].cate_id+"'>"+lang.edit+"</a>  | <a href='javascript:if(confirm(\""+lang.confirm_delete+"\"))window.location =\"index.php?app=gcategory&amp;act=drop&amp;id="+res[i].cate_id+"\";'>"+lang.drop+"</a>" + add_child + "</a>|<a href='javascript:attr("+res[i].cate_id+")'>"+lang.gcategory_attr+"</a></span></td></tr>";
                    }
                    pr.after(str);
                    change_background();
                    $('span[ectype="inline_edit"]').unbind('click');
