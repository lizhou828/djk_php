...
//     var url='index.php?app=datacache&act=cache_json_category';
//       $.getJSON(url,function(json){
//        //json  你通过 url 取得的json数据.
//        var category = json;
//        for (var i=0;i<category.length;i++) {
////          alert(category[i].pds);
//        }
//    });
});
...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
        ...
        if(SPEC.spec_name_1){
            ...
            tpl.find('*[item="spec_1"]').append('<input type="hidden" aname="spec_1[]" name="spec_1['+ item.spec_id +']" value="' + item.spec_1 + '" />');
        }else{
            tpl.find('*[item="spec_1"]').append('<input type="hidden" aname="spec_1[]" name="spec_1['+ item.spec_id +']" value="" />');
            (SPEC.spec_qty == "1" || SPEC.spec_qty == "0") && tpl.find('*[item="spec_1"]').hide();
        }
        if(SPEC.spec_name_2){
            tpl.find('*[item="spec_2"]').text(item.spec_2);
            tpl.find('*[item="spec_2"]').append('<input type="hidden" aname="spec_2[]" name="spec_2['+ item.spec_id +']" value="' + item.spec_2 + '" />');
        }else{
            tpl.find('*[item="spec_2"]').append('<input type="hidden" aname="spec_2[]" name="spec_2['+ item.spec_id +']" value="" />');
            (SPEC.spec_qty == "1" || SPEC.spec_qty == "0") && tpl.find('*[item="spec_2"]').hide();
        }
        tpl.find('*[item="price"]').append('<input type="hidden" aname="price[]" name="price['+ item.spec_id +']" value="' + item.price + '" />' + item.price);
        tpl.find('*[item="org_price"]').append('<input type="hidden" aname="org_price[]" name="org_price['+ item.spec_id +']" value="' + item.org_price + '" />' + item.org_price);

        //tpl.find('*[item="bili"]').append('<span>'+(((1-(parseFloat(item.org_price)/parseFloat(item.price))).substring(0,(1-(parseFloat(item.org_price)/parseFloat(item.price)).indexOf(".") + 3))*100+"%")+'</span>');
        var str_val =((1-item.org_price/item.price)*100).toFixed(2);
        tpl.find('*[item="bili"]').append('<span>'+str_val+'%</span>');
...
function drop_image2(file_path)
{
    if (confirm(lang.uploadedfile_drop_confirm))
    {
        var url = SITE_URL + '/index.php?app=my_goods&act=drop_image';
        $.getJSON(url, {'file_path':file_path}, function(data){
            if (data.done)
            {
                $('*[file_path="' + file_path + '"]').remove();
                set_cover($("#desc_images2 li:first-child").attr('file_id'));
            }
            else
            {
                alert(data.msg);
...
function spec_editor(){
    ...
    spec_item && $.each(spec_item,function(i,item){ // 遍历生成规格项
        ...
        tpl.find('*[item="org_price"]').val(item.org_price);
        tpl.find('*[item="bili"]').html(item.bili);
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
            ...
            var org_price = $.trim($(this).find('*[item="org_price"]').val());
            var price = $.trim($(this).find('*[item="price"]').val());
//            var bili =(((1-(parseFloat(org_price)/parseFloat(price))).toFixed(4))*100+"%");
            ...
            /* 判断进货价格非法 */
//            if(isNaN(org_price) || org_price <0 || !org_price){
//                valid && org_price_rror.push(item);
//            }
        ...
        /* 判断价格 */
        if(org_price_error.length>0){
            var msg = lang.follow_spec_price_invalid + '\n';
            $.each(org_price_error,function(i,val){
                msg += val + '\n';
            });
