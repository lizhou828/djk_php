...
class GcategoryModel extends BaseModel
{
    ...
    var $_relation  = array(
        ...
        'belong_to_pd' => array(
            'model' => 'pdcategory',
            'type' =>  HAS_AND_BELONGS_TO_MANY,
            'middle_table'  => 'pd_category',
            'reverse' =>'belongs_to_pdcategory'
        ),
    );
...
* @return array
*/
    function get_list2($parent_id = -1, $shown = false)
    {
        $conditions = "1 = 1 and dropState=1 ";
        $parent_id >= 0 && $conditions .= " AND parent_id = '$parent_id'";
        $shown && $conditions .= " AND if_show = 1";
        ...
        return $this->find(array(
            'conditions' => $conditions,
            'order' => 'sort_order, cate_id DESC',
            'fields' => "this.*,(select count(1) from ecm_goods g where g.cate_id_1 =gcategory.cate_id or g.cate_id_2 =gcategory.cate_id or g.cate_id_3 =gcategory.cate_id or g.cate_id_4=gcategory.cate_id) as \"goodscount\""
        ));
    }
    ...
    function _get_descendant($ids, &$ids_total)
    {
        $childs = $this->find(array(
            'fields' => 'cate_id',
            'conditions' => "parent_id " . db_create_in($ids)."and dropState=1"
        ));
...
class GcategoryBModel extends GcategoryModel
{
    ...
    {
        $conditions = "parent_id = '$parent_id' AND cate_name = '$cate_name' and dropState=1";
    ...
    function _getConditions($conditions, $if_add_alias = false)
    {
        ...
        $q="and dropState=1";
        return $res.$q ? $res . " AND {$alias}store_id = '{$this->_store_id}'" : " WHERE {$alias}store_id = '{$this->_store_id}'".$q;
    }

    function is_leaf($id)
    {
        $sql = "SELECT COUNT(*) FROM {$this->table} " . $this->_getConditions("parent_id = '$id'")." and dropState=1";
    ...
    function get_children($id, $cached = false)
    {
        ...
        {
            $sql = "SELECT cate_id, cate_name FROM {$this->table} WHERE parent_id = '$id' AND store_id = '{$this->_store_id}' and dropState=1";
            $res = $this->getAll($sql);
        }
   ...
   //覆盖父类get
    function get($cate_id) {
        $sql = "select * from {$this->table} where cate_id=".$cate_id." and dropState=1 and if_show=1";
