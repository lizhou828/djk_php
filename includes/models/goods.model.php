...
class GoodsModel extends BaseModel
{
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true)
    {
        ...
        $fields = "g.goods_id,g.pd_id, g.store_id, g.type, g.goods_name,g.org_price, g.cate_id, g.cate_name, g.brand_id,g.brand, g.spec_qty, g.spec_name_1, g.spec_name_2, g.if_show, g.closed, g.add_time, g.recommended,g.default_image,g.dropState, g.start_time,g.end_time," .
            ...
            "gst.views, gst.sales, gst.comments,s.store_name as \"store_name\"";
        ...
        $tables = "{$this->table} g " .
            ...
            "LEFT JOIN {$goodsattr_mod->table} gd ON g.goods_id = gd.goods_id ".
            "LEFT JOIN {$goodsimage_mod->table} gi ON g.goods_id = gi.goods_id" ;
        /* 条件(WHERE) */
		$conditions = $this->_getConditions($conditions, true);

        if($attrid && $attrid!=null)
        {
            $conditions .= " AND gd.attr_id IN (".$attrid.")";
        }

        if ($attrvalue && $attrvalue!=null)
        {
            if (strpos($attrvalue,',')) {
                $attr_str =  explode(',',$attrvalue);
                if (count($attr_str)>0) {
                    foreach ($attr_str as $ar) {
                            $conditions.=" and exists (select 0 from ecm_goods_attr ga  where g.goods_id = ga.goods_id and ga.attr_value= '$ar')";
                        }
                    }


            }else {
                $conditions.= " AND gd.attr_value = '$attrvalue'";
            }
        }

//       if (!strpos($conditions,' g.dropState =2 ')) {
          $conditions .= " AND gs.spec_id IS NOT NULL AND s.store_id IS NOT NULL and s.state = 1";
//       }

//        $conditions .= " AND gs.spec_id IS NOT NULL AND s.store_id IS NOT NULL and g.dropState <>0";
...
//            $conditions .= " AND g.goods_id " . db_create_in($goods_ids);
            ...
            $conditions .= " AND g.cate_id_1" . db_create_in($scate_ids);
        }

        $pos = strpos($order,'views');
        /* 排序(ORDER BY) */
        if ($pos !== false || !$order)
        {
            $order = ' ORDER BY  g.org_price/g.price  desc ';
        } else {
        ...
        if ($count)
        {
            $this->_updateLastQueryCount("SELECT   COUNT(*) AS c   FROM(SELECT  g.* FROM {$tables}{$conditions}  GROUP BY g.goods_id ) AS b");

        }

        /* 完整的SQL */
        $this->temp = $tables . $conditions;
        $sql = "SELECT {$fields} FROM {$tables}{$conditions} GROUP BY g.goods_id {$order}{$limit}";
    ...
    function get_info($id)
    {
        ...
        $goods = $this->get(array(
            'conditions' => "goods_id = '$id' and g.dropState<>0 ",
...
//            unset($specs[$goods['default_spec']]);
    ...
    function get_count_of_store($store_id)
    {
        ...
        if (!isset($data[$store_id]))
        {
            ...
            if($data === false)
            {
                $sql = "SELECT store_id, COUNT(*) AS goods_count FROM {$this->table} WHERE if_show = 1 AND dropState=1 and closed = 0 GROUP BY store_id";
                $data = array();
     ...
     */
    function findByPage($sql,$currentPage,$perPage){
        $default_image = true;
        $goods_list=array();
        if(isset($sql) && isset($currentPage) && isset($perPage)){
            $startResult = ($currentPage-1)*$perPage;
            ...
            echo "your sql sentences is illegal!";
            return;
        }
        return $goods_list;
    }
...
class GoodsBModel extends GoodsModel
{
    ...
    function add($data, $compatible = false)
    {
        ...
        if(empty($data['store_id']) || $data['store_id'] =="")
        {
    ...
    function _getConditions($conditions, $if_add_alias = false)
    {
        ...
        if(!empty($_POST["store_id"]) && $_POST["store_id"]!="")
        {
        ...
        /*
        if($_GET["app"]=="service" && ($_GET["act"]=="edit" || $_GET["act"]=="my_goods" || $_GET["act"]=="query_goods" || $_GET["act"]=="kaidan")) {
            return $res;
        }*/
        if($_GET["app"]=="service")
        {
        ...
        return $res ? $res . " AND {$alias}store_id = '{$store_id}'" : " WHERE {$alias}store_id = '{$store_id}'";
    }
    ...
    function get_count()
    {
        $sql = "SELECT COUNT(*) FROM {$this->table} WHERE store_id = '{$this->_store_id}' and dropState=1 ";
