...
class RegionModel extends BaseModel
{
    ...
    {
        $conditions="and dropState=1";
        if ($parent_id >= 0)
        {
            return $this->find(array(
                'conditions' => "parent_id = '$parent_id'".$conditions." and region.dropState=1",
        ...
        {
            return $this->find(array(
                'conditions' => $conditions." and region.dropState=1",
                'order' => 'sort_order, region_id',
            ));
        }
    }
    ...
    {
        $conditions = "parent_id = '" . $parent_id . "' AND region_name = '" . $region_name . "' and region.dropState=1";
    ...
    function _get_descendant($ids, &$ids_total)
    {
        $childs = $this->find(array(
            'fields' => 'region_id',
            'conditions' => "parent_id " . db_create_in($ids)." and region.dropState=1"
        ));
