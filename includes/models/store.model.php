...
class StoreModel extends BaseModel
{
    ...
    {
        $conditions = "store_name = '" . $store_name . "' and dropState=1";
    ...
    {
        $info = $this->get(array(
            'conditions' => 's.store_id='.$store_id.' and s.dropState=1',
            'join'       => 'belongs_to_user',
            'fields'     => 'this.*,member.user_name, member.email,(select user_name from '.DB_PREFIX.'member where user_id=member.t_id) as t_id',
        ));
...
//        }
     ...
     * 获得邀请码
     * **/
    function get_RPC_User($str)
    {
        $data = array();
        $sql = "SELECT user_name FROM ".DB_PREFIX."member WHERE status='2' and user_name LIKE '$str%' LIMIT 10";
        $res = $this->db->query($sql);
        while ($row = $this->db->fetchRow($res))
        {
  ...
  * 获得未激活的服务中心
  * **/
    function get_RPC_User2($str)
    {
        $data = array();
        $sql = "SELECT user_name FROM ".DB_PREFIX."member WHERE status in (0,1,2) and user_name LIKE '$str%' LIMIT 10";
