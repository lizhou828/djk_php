...
class BaseOrder extends Object
{
    ...
    function _get_my_address($user_id)
    {
        ...
        return $address_model->find('user_id=' . $user_id.' and region_id is not null and region_name is not null order by addr_id desc');
    }
    ...
    function _gen_order_sn()
    {
        ...
        //$order_sn = date('ymdHis').$time_s.$this->generate_code(2);
        $order_sn = date('ymdHis').$time_s;
...
//        }
    ...
    function _handle_order_info($goods_info, $post)
    {
        ...
        //$order_status=ORDER_SUBMITTED;       //暂时改成生成订单就付款
        ...
        $jine = 0;
//        $ticheng = 0;
        if(count($goods_info['items'])>0) {
            foreach ($goods_info['items'] as $goods) {
                if ($goods['org_price'] > 0 && doubleval($goods['org_price']) < doubleval($goods['price'])) {
                    $jine += (doubleval($goods['price'])*$goods['quantity'])  - (doubleval($goods['org_price'])*$goods['quantity']);
                } else if ($goods['ticheng']>0){
//                    $ticheng = $goods['ticheng'];
                    $jine += $goods['amount']*$goods['ticheng'];
                } else {
                    $model_store =& m('store');
                    $userInfo    = $model_store->get_info(intval($goods_info['store_id']));
                    //提成比例
                    $ticheng=$userInfo["ticheng"];
                    $jine += $goods['amount']*$ticheng;
                }
            }
        ...
        return array(
            ...
            'postscript'   => isset($post['postscript']) ? $post['postscript'] : null ,
//            'ticheng'       =>  $ticheng,
            'jine'          =>  $jine,
            'session_id'  =>  $post['session_id'],
            'guojiguan' => isset($goods_info['guojiguan']) ? $goods_info['guojiguan']:1,
            'shop_id'   => isset($goods_info['shop_id']) ? $goods_info['shop_id'] : null,
        );
...
//        $shipping_info  = $shipping_model->get("store_id={$goods_info['store_id']} AND enabled=1");
...
//            'shipping_name' =>  addslashes($shipping_info['shipping_name']),
    ...
    function _get_regions()
    {
        ...
        $regions = $model_region->get_list(1);
        if ($regions)
        {
            $tmp  = array();
