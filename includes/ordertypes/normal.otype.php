...
//        $template = 'order.form.html';
        ...
        if (empty($data['shipping_methods']))
        {
            ...
            return false;*/
            //如果没有配送方式,默认卖家包邮
            $data['shipping_methods']   = array(0=>array(
                "shipping_id"=>0,
                "store_id"=>0,
                "shipping_name"=>"卖家包邮",
                "shipping_desc"=>"卖家包邮",
...
//        $this->print_r($base_info);
        ...
        foreach ($goods_info['items'] as $key => $value)
        {
            $goods_items[] = array(
                ...
                'jifen'=> $value['if_jifen']*$value['quantity'],
    ...
    function get_address() {
        $visitor =& env('visitor');
        return  $this->_get_my_address($visitor->get('user_id'));
    }

    function get_address2($user_id) {
        return  $this->_get_my_address($user_id);
    }

    function get_regions(){
        return   $this->_get_regions();
    }
}
