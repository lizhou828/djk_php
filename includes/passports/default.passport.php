...
class DefaultPassportUser extends BasePassportUser
{
    ...
    /* 注册 */
    function register($user_name, $password, $email,$t_id,$type)
    {
    ...
    function edit($user_id, $old_password, $items, $force = false)
    {
        ...
        if (isset($items['password2']))
        {
            $edit_data['password2']  = md5($items['password2']);
        }
    ...
    function get($flag, $is_name = false)
    {
        if ($is_name)
        {
            $conditions = "(user_name='{$flag}' or phone_mob='{$flag}' or email='{$flag}' ";
            if (is_numeric($flag)) {
                $conditions = $conditions. " or user_id='{$flag}' ";
            }
    ...
    function auth($user_name, $password)
    {
        ...
        if($info["status"]!="2" || $info["dropState"]!=1 )
        {
            $this->_error('auth_failed_byStatus');
            return 0;
        }
