(function() {
	$(".proCitySelAll").click(function(event) {
		if ($("body").data("CitysAll") == null) {
			sendAllCitiesAjax(sendAllCitiesAjax_URL);
		}
		$(this).select();
		$(".provinceCity").hide();
		...
		var t2 = o2.top;
		var h2 = $(this).height();
		$(".provinceCityAll").css("top", t2 + h2 +2).css("left", l2).toggle();
		...
		if ($("body").data("allProvinces") == null) {
			sendAllProvinceAjax(sendAllProvinceAjax_URL);
		}
		if ($("body").data("allCountys") == null) {
			sendAllCountiesAjax(sendAllCountiesAjax_URL);
		}
		...
		if ($(this).hasClass("proCityQuery"))
		 {
			...
			if ($("body").data("allExistCitys") == null) {
				sendCitiesAjax(sendCitiesAjax_URL);
			}
...
function sendAllProvinceAjax(url) {
	$.ajax({
		type: "get",
		url: url,
		async: false,
...
function sendAllCitiesAjax(url) {
	$.ajax({
		type: "get",
...
function sendAllCountiesAjax(url)
 {
	$.ajax({
		type: "get",
		url: url,
...
function viewAllHotCities() {
	...
	function(i, city) {
		if (city.hotCity) {
            if($(".hotCityAll .list ul").children().length<=15)
            {
...
function viewAllProvince(page) {
    ...
    for (var i = start; i < end; i++) {
        ...
        if( p_id == 4 ||
            ...
            p_id == 25 ||
            p_id == 31 ){
             li = $('<li><a style="background: none repeat scroll 0% 0% transparent; border: 0px none;color: red" href="javascript:onclick=viewAllCities(' + i + ');" id="' + p_id + '">' + p_name + '</a></li>');
        }else{
             li = $('<li><a style="background: none repeat scroll 0% 0% transparent; border: 0px none;color:#d5d2d2" href="javascript:alert('+"'该区暂未开放,请选择其他的红色已开放的区域'"+')" id="' + p_id + '">' + p_name + '</a></li>');
        }
        $(".provinceAll .list ul").append(li);
    }
    $(".provinceAll .list #provincePage1").remove();
...
function allCountyPage(page) {
	nameOfProvince = $("body").data("pAllName");
	cityCurrentName = $("body").data("nameOfCityAll");
	$("input.current2").removeClass("iGrays");
    if(nameOfProvince!=undefined && cityCurrentName!=cityCurrentName){
	...
	for (var j = start; j < end; j++) {
		var c_id = countiesAll[j].id;
		var countyName = countiesAll[j].areaName.substr(0, 4);;
		var li = $('<li><a href="javascript:onclick=addrInputAll(' + j + ')" id="' + c_id + '">' + countyName + '</a></li>');
