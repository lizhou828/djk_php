...
//        exit;
        ...
        /*浏览历史*/
        $goods_app = new GoodsApp();
        $this->assign('goods_history', $goods_app->_get_goods_history(0));
...
//            $goodsattr_mod = & m('goodsattr');
            ...
            $sql = "SELECT COUNT(*) FROM {$table} WHERE" . $conditions." and g.dropState =1 and g.if_show=1";
...
//                        $conditions.=" and g.cate_id=".$param['cate_id'];
                        ...
                        $gcategory = $gcategory_mode->get($param['cate_id']);
                        $sql = "SELECT g.cate_id_" . ($layer) . " AS id, COUNT(*) AS count FROM {$table} WHERE g.cate_id_".($layer-1)."=".$gcategory['parent_id']." and g.cate_id_" . ($layer) . " > 0 and g.dropState=1 and g.if_show=1 GROUP BY g.cate_id_" . ($layer) . " ORDER BY count DESC";
                    } else {
