...
//            echo "发送失败，请不要重复发送!<script>window.clearInterval(InterValObj);$(\"#btnSendCode\").removeAttr(\"disabled\");$(\"#btnSendCode\").val(\"重新发送验证码\");</script>";
//            exit;
//        }
...
//        $code= 111111;
    ...
    function sendMsgCode(){
        ...
        $code= $this->generate_code(6);
        $_SESSION["yanzhengma"]=$code;
        $result =  $this->send_msm($phone_mob,$code);
        echo $result;
        exit;
    }
    ...
    function send_msm($tel,$code){
        ...
        if($rs["errorno"]==0){
            ...
            return "发送失败！errorCode:".$rs["errorno"]."  失败原因：".$rs["info"];
            $_SESSION["msm_time"]=0;
        }
    }
            ...
            if(count($order_goods) > 0){
                foreach($order_goods as $goods){
                    if($goods['cellphone'] != 0 && $goods['cellphone'] != ""){
                       $code = $this -> generate_code(6);
                       $smsclient = new SMSClient();
                       $smsclient -> sendSMS($goods['cellphone'],'您好，商品:'.$goods['goods_name'].'。验证码是：'.$code." 。本短信发自中国大集客！");
