...
//                'im_aliww'  => $_POST['im_aliww'],
            ...
            /* 到用户系统中注册 */
            //$user_id = $ms->user->register($user_name, $password, $email, $data,$type);
...
//				"region_id"=>$region_id,
            ...
            if(!empty($fwzx_jibie) && $fwzx_jibie!="")
            {
                ...
                if($s_info["id"]<=0)
                {
                    $serviceInfo_model->add(array("service_id"=>$id,"service_jibie"=>"$fwzx_jibie"));
                }else
                {
                    $serviceInfo_model->edit($s_info["id"],array("service_jibie"=>"$fwzx_jibie"));
                }
