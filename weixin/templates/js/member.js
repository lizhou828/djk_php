$(function(){
    ...
    $('*[ectype="gselector_service"]').focus(function(){
        var id = $(this).attr('gs_id');
        var name = $(this).attr('gs_name');
        var callback = $(this).attr('gs_callback');
        var type = $(this).attr('gs_type');
        ...
        var title = $(this).attr('gs_title') ? $(this).attr('title') : '';
        var width = $(this).attr('gs_width');
        ajax_form(id, title, SITE_URL + '/index.php?app=service&act=gselector&dialog=1&title=' + title + '&store_id=' + store_id+ '&id=' + id + '&name=' + name + '&callback=' + callback, width);
        return false;
    });
...
function insert_editor(file_name, path)
{
    tinyMCE.execCommand('mceInsertContent', false,
        '<img src="'+ IMG_URL +'/' + path + '" alt="'+ file_name + '">');
}
...
function trigger_uploader(){
    ...
    $('*[ectype="handle_pic"]').find('img:first').hover(function(){
        ...
        drop = handler.find('*[ectype="drop_image2"]');
        drop.unbind('click');
        drop.click(function(){
            drop_image2(handle_pic.attr("file_path"));
        });

        drop = handler.find('*[ectype="drop_image3"]');
        drop.unbind('click');
        drop.click(function(){
            drop_image3(handle_pic.attr("file_id"));
        });
...
function drop_image3(file_id)
{
    if (confirm(lang.uploadedfile_drop_confirm))
    {
        var url = SITE_URL + '/index.php?app=my_goods&act=drop_image2';
...
function setGoods(id,title,price,cate_name,stock,store_id)
{
  var goos_info=new Object();
  goos_info.id=id;
  goos_info.title=title;
  goos_info.price=price;
  goos_info.cate_name=cate_name;
  goos_info.stock=stock;
  goos_info.store_id=store_id;
  return goos_info;
}
